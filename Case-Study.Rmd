...
```
Next, we will check if the rate of unemployed citizens and the share of youth/immigrants correlate. We achieve this by testing these statistics in relation to the total number of citizens within the separate districts. On the left plot we see that the number of young perople correlates strongly with the number of unemployed citizens. But this is clear, what is a cofounding factor for it? Obviously the number of citizens. Let us rather take the rate of unemplyoment and the share of young citizens instead of the total numbers. The same holds for number of immigrants.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
share_youth_plot_1 <- ggplot(unemployment, aes(number_citizens_youth, number_unemployed)) + geom_point() + geom_smooth(method="lm") + labs(x="Number of Young citizens", y = "Number of unemployed Citizens", title = "6. Correlation of Number of young citizens/ unemployed citizens") + theme(plot.title = element_text(size=10))


share_youth_plot <- ggplot(unemployment, aes(share_youth, rate_unemployed)) + geom_point() + labs( x= "Share youth", y= "Rate unemployed", title= "7. Correlation of rate of unemployment with share of youth")+
geom_smooth(method="lm") + theme(plot.title = element_text(size=10))



immigrants_plot_1 <- ggplot(unemployment, aes(number_immigrants, number_unemployed)) + geom_point() + geom_smooth(method="lm") + labs(x="Number of Immigrants", y = "Number of unemployed Citizens", title = "8. Correlation of Number of Immigrants/ unemployed citizens") + theme(plot.title = element_text(size=10))

immigrants_plot <- ggplot(unemployment, aes(share_immigrants, rate_unemployed)) + geom_point() + labs( x= "Share immigrants", y= "Rate unemployed", title= "9. Correlation of rate of unemployment with share of immigrants")+ geom_smooth(method="lm") + theme(plot.title = element_text(size=10))
```
...
```{r}
suppressWarnings(print(share_youth_plot_1))
suppressWarnings(print(share_youth_plot))
```


```{r, test1, fig.show="hold", out.width="50%"}
suppressWarnings(print(immigrants_plot_1))
suppressWarnings(print(immigrants_plot))
```
...
The demonstrative plot 8 shows that a difference in the life expectancy of females and male exists. In general females live longer than men. But the question arises whether the difference arose by any chance. 
For this we run a Permutation test. Plot 9 shows the clear result underlying our hypothesis.
...
p1 <- ggplot(life_expect_copy, aes(Gender, life_expectancy)) + geom_boxplot()+labs(x="gender", y="Life expectancy", title="10. Life expectancy per gender"
...
p2 <- ggplot(data.table(T_star = T_star), aes(T_star)) + geom_histogram(bins=30) +
  geom_vline(aes(xintercept=T_ref, color="T_ref")) + labs(title = "11. Difference in medians after repeating permutation 1000 times") + theme(plot.title = element_text(size=10))
```

```{r, figures-side, fig.show="hold", out.width="50%", results='hide', message=FALSE}
